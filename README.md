...
- **多语言分类与翻译合成**：音色可标注语种（`language`），WebUI 按语种分组管理；合成前自动把「非目标语种」文本翻译成目标语种（默认汉语），本地检测语种零额外消耗，翻译接口全字段可配置。详见下方「翻译合成」。
- **括号内容不朗读**：正文被 `（）()` `[]` 包裹的文字不进入语音；both 合并模式随文字显示、仅语音不念，`voice_only`/不合并 both 时括号内容单独作为一条文字发给用户，保证看得见。
...
| `/tts <文本>` | 临时朗读某段文本 | ❌ 一次性 | 念一次，不开启语音模式。例：`/tts 你好呀`。**整段一次合成、只发一条语音**（不分段、也不补发文字） |
| `/tts0 <文本>` | 同 `/tts` | ❌ 一次性 | 整段一条语音，保留旧用法（v2.1.49 起与 `/tts` 行为一致） |
| `/tts1 <文本>` | 按换行分块逐条念 | ❌ 一次性 | 多行文本按换行分块，每块一条语音；过短的行（< `newline_min_len` 字，默认 10）会与相邻行合并 |
...
- **概览**：服务端健康（各节点在线/冷却剩余秒/异常次数）、全局配置（自动语音/发送方式/范围/默认音色/采样率）、统计。
- **音色管理**：按**语种分组**展示与筛选，搜索、内联试听（顶部输入文本点任意音色即播放）、**新增/编辑/删除（二次确认）**与批量显示/隐藏/删除，每条含参考音频/参考文本/语种标签。
- **翻译合成**：翻译总开关、目标语种（默认 `zh`）、需翻译语种白名单（留空=除目标语种外全部）；通用翻译 API 适配器（URL/方法/API Key/认证头/scheme/额外请求头/参数模板 `{text}` `{source}` `{target}`/响应解析路径/超时）全字段配置，含「测试」显示检测语种与译文。配置存插件自有 `data/translate_config.json`，保存即热生效。
- **会话管理**：按会话（origin）查看/切换语音开关、音色、发送方式，搜索 + 批量关闭。
- **配置**：当前生效配置分组只读展示（服务端 / 语音行为 / 分段与重试），修改请到 AstrBot 配置弹窗。

**与配置弹窗的关系（解耦）**：

- 会话开关/会话音色/发送方式、WebUI 设置的默认音色，均存插件自有 `data/tts_*.json`（与聊天指令 `/tts_on`、`/tts_voice` 同源），**重启不丢、热生效**，与 AstrBot 主配置解耦。
- **WebUI 里新增/编辑/删除的音色** 存插件自有 `data/tts_voices_lib.json`，优先于 AstrBot 配置弹窗中的 voices（同名的以 WebUI 为准）。配置弹窗中的音色仍可显示与试听，但增删改只在 WebUI 音色库中生效；配置型音色想删需去配置弹窗。
- 试听每次合成后自动下载 wav 到本地，不依赖服务端直链端点。
...
| `send_mode` | `both` | `both` / `voice_only` |
| `skip_bracket_tts` | `true` | **括号内容不朗读**：开启后正文被 `（）()` `[]` 包裹的文字不进入语音（详见下方说明） |
| `split_by_newline` | `true` | 多行回复优先按换行分块合成（避免把换行念成噪音/静音） |
| `newline_min_len` | `10` | **换行分段最小字数**：按换行切出的块短于此字数时与相邻块合并，避免「嗯」「好的」这类极短行各占一条语音（`0`=不合并）；`/tts1` 同样受此值控制 |

### 翻译合成配置

翻译合成在 WebUI「翻译合成」面板配置（**不在此处**），保存后写入插件自有 `data/translate_config.json`，**热生效、重启不丢**，与 AstrBot 主配置解耦：

- **总开关 `enabled`**：关闭则完全不翻译，文本原样合成。
- **目标语种 `target`**：翻译成什么（默认 `zh` 汉语）。
- **需翻译语种 `source`**：白名单，留空 = 除目标语种外的所有语种都翻译；填了则只翻译名单内的语种。语种判定走本地 Unicode 脚本检测（中文/日文/韩文/俄文/泰文/阿拉伯/梵文等），零 API 消耗。
- **翻译 API 适配器**（通用，不缺胳膊少腿）：
  - `url`：请求地址；`method`：GET/POST；`content_type`：JSON / 表单。
  - 认证：`apikey` + 认证头名（默认 `Authorization`）+ scheme（默认 `Bearer`），最终请求头为 `认证头名: scheme apikey`。
  - `extra_headers`：额外请求头（key/value，可含 `{text}`/`{source}`/`{target}`）。
  - `params`：请求参数（key/value），值支持占位符 `{text}`（待翻译文本）、`{source}`（源语种）、`{target}`（目标语种）。
  - `response_path`：从响应 JSON 取译文的路径，点号+数组下标，如 `data.trans_result[0].dst`。
  - `timeout`：超时秒数。
- 任意异常兜底回退原文，绝不影响合成主流程；译文带缓存。

### 括号内容不朗读（`skip_bracket_tts`）

开启后，正文中被括号包裹的文字（支持中文 `（）`、英文 `()`、方括号 `[]`）**不进入语音合成**：

- `both` 合并模式：括号内容随文字正常显示在聊天记录里，仅语音不念。
- `voice_only` 或不合并 `both`：文字不在聊天记录中，括号内容会**单独作为一条文字消息**发给用户，保证看得见。
- `/tts`、`/tts0`、`/tts1` 指令同样遵循「括号不念」：仅剥离括号，且指令**只发语音、不补发文字**（v2.1.50 起；发送方式 `/tts_type`、`send_mode` 只作用于自动语音链路）。
